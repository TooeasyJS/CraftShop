  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden bg-[#050905]">
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 gap-8 max-w-3xl w-full">
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.9 }} className="flex flex-col sm:flex-row gap-4 items-center">
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    if (saved) { try { setDiscordUser(JSON.parse(saved)); } catch { localStorage.removeItem("discord_user"); } }
  }, []);

  const handleLogout = () => { setDiscordUser(null); localStorage.removeItem("discord_user"); };
